...
# Variables
sleep_data_file_path = "data\sleep_study\cmu-sleep.csv"
...
def create_sleep_heatmap(sleep_data):
    columns_to_keep = ['study', 'demo_firstgen', 'bedtime_mssd', 'TotalSleepTime', 'midpoint_sleep', 'frac_nights_with_data', 'daytime_sleep', 'score_scaled']
    sleep_data_copy = sleep_data[columns_to_keep]
    plotting.plot_correlation_matrix(sleep_data_copy , "Score")

def create_sleep_scatter_plots(sleep_data):
    plotting.scatterplot_regression_by_group(sleep_data, 'study', 'midpoint_sleep', 'score')
    plotting.scatterplot_regression_by_group(sleep_data, 'study', 'bedtime_mssd', 'score')
    plotting.scatterplot_regression_by_group(sleep_data, 'study', 'TotalSleepTime', 'score')

def run_full_sleep_analysis():
    sleep_data = cleaning.clean_sleep_data(sleep_data_file_path)
    create_new_features(sleep_data)
    create_sleep_heatmap(sleep_data)
    create_sleep_scatter_plots(sleep_data)
